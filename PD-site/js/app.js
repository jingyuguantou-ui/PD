...
function init() {
  ...
  // ===== TABLET UI: Procreate-style =====
  const isTabletUI = document.body.classList.contains("tablet-forced") || window.matchMedia("(pointer: coarse) and (min-width: 481px)").matches;
  if (isTabletUI) {
    const menuBtn = document.getElementById("menuBtn");
    const tabletMenu = document.getElementById("tabletMenu");
    const panelOverlay = document.getElementById("panelOverlay");
    const tabletActionbar = document.getElementById("tabletActionbar");
    const tabletExportSheet = document.getElementById("tabletExportSheet");
    const tabletEditbar = document.getElementById("tabletEditbar");
    ...
    function closeAllTabletPanels() {
      if (tabletMenu) tabletMenu.classList.remove("open");
      if (controls) controls.classList.remove("panel-open");
      if (panelOverlay) panelOverlay.classList.remove("visible");
      if (tabletExportSheet) tabletExportSheet.classList.remove("open");
    }
    function openMenu() {
      closeAllTabletPanels();
      if (tabletMenu) tabletMenu.classList.add("open");
      if (panelOverlay) panelOverlay.classList.add("visible");
    }
    function openSettings() {
      closeAllTabletPanels();
      if (controls) controls.classList.add("panel-open");
      if (panelOverlay) panelOverlay.classList.add("visible");
    }
    function openExportSheet() {
      if (!state.result) return;
      closeAllTabletPanels();
      if (tabletExportSheet) tabletExportSheet.classList.add("open");
      if (panelOverlay) panelOverlay.classList.add("visible");
    }
    ...
    if (tabMenu) tabMenu.addEventListener("click", openMenu);
    if (panelOverlay) panelOverlay.addEventListener("click", closeAllTabletPanels);
    if (sheetCancel) sheetCancel.addEventListener("click", closeExportSheet);

    // Menu items
    const menuPage = tabletMenu ? tabletMenu.querySelectorAll("[data-page]") : [];
    menuPage.forEach((btn) => {
    ...
    const menuLibBtn = document.getElementById("menuLib");
    if (menuLibBtn) menuLibBtn.addEventListener("click", () => { closeAllTabletPanels(); openLibrary(); });
    const menuHistBtn = document.getElementById("menuHist");
    if (menuHistBtn) menuHistBtn.addEventListener("click", () => { closeAllTabletPanels(); History.show(); });
    const menuCalcBtn = document.getElementById("menuCalc");
    if (menuCalcBtn) menuCalcBtn.addEventListener("click", () => { closeAllTabletPanels(); if (typeof Features !== "undefined") Features.showCalc(); });
    const menuDarkBtn = document.getElementById("menuDark");
    if (menuDarkBtn) menuDarkBtn.addEventListener("click", () => {
    ...
    if (menuHelpBtn) menuHelpBtn.addEventListener("click", () => { closeAllTabletPanels(); if (typeof Tutorial !== "undefined") Tutorial.show(); });
    const menuSCBtn = document.getElementById("menuSC");
    if (menuSCBtn) menuSCBtn.addEventListener("click", () => { closeAllTabletPanels(); Shortcuts.show(); });
    const menuVerBtn = document.getElementById("menuVer");
    if (menuVerBtn) menuVerBtn.addEventListener("click", () => { closeAllTabletPanels(); Versions.show(); });

    // Theme in menu
    tabletMenu.querySelectorAll(".theme-pick-inline .seg").forEach((b) => {
      b.addEventListener("click", () => {
        applyTheme(b.dataset.theme);
        tabletMenu.querySelectorAll(".theme-pick-inline .seg").forEach((s) => s.classList.toggle("active", s.dataset.theme === b.dataset.theme));
        document.querySelectorAll(".theme-pick .seg").forEach((s) => s.classList.toggle("active", s.dataset.theme === b.dataset.theme));
      });
    });
    // Language in menu
    tabletMenu.querySelectorAll(".lang-pick-inline .seg").forEach((b) => {
      b.addEventListener("click", () => {
        I18N.setLang(b.dataset.lang);
        tabletMenu.querySelectorAll(".lang-pick-inline .seg").forEach((s) => s.classList.toggle("active", s.dataset.lang === b.dataset.lang));
        document.querySelectorAll(".lang-pick .seg").forEach((s) => s.classList.toggle("active", s.dataset.lang === b.dataset.lang));
        I18N.applyTranslations();
      });
    });
    ...
    // Tab action bar
    if (tabGen) tabGen.addEventListener("click", () => { if (!tabGen.disabled) run(); });
    if (tabEdit) tabEdit.addEventListener("click", () => {
      if (tabEdit.disabled) return;
      const editBtn = document.getElementById("editToggle");
      if (editBtn) editBtn.click();
    });
    if (tabExport) tabExport.addEventListener("click", openExportSheet);
    if (tabLib) tabLib.addEventListener("click", () => openLibrary());

    // Edit done
    if (editDoneBtn) editDoneBtn.addEventListener("click", () => {
      const editBtn = document.getElementById("editToggle");
      if (editBtn) editBtn.click();
    });

    // Export sheet actions
    tabletExportSheet.querySelectorAll(".sheet-btn[data-action]").forEach((btn) => {
      btn.addEventListener("click", () => {
        closeExportSheet();
    ...
    // Show tablet action bar
    if (tabletActionbar) tabletActionbar.style.display = "";

    // Override setTool to show/hide edit bar
    const _origSetTool = setTool;
    setTool = function(tool) {
      _origSetTool(tool);
      if (tabletEditbar) {
        const inEdit = state.editing && tool !== "view";
        tabletEditbar.classList.toggle("visible", inEdit);
      }
    };

    // Update action bar state on run
    const _origRun = run;
    run = function() {
      _origRun();
      if (tabGen) tabGen.disabled = !state.img;
      if (tabEdit) tabEdit.disabled = !state.result;
      if (tabExport) tabExport.disabled = !state.result;
    };

    // Update on edit toggle
    const editToggle = document.getElementById("editToggle");
    if (editToggle) {
      const obs = new MutationObserver(() => {
        const inEdit = editToggle.classList.contains("active");
        if (tabletEditbar) tabletEditbar.classList.toggle("visible", inEdit);
        if (tabEdit) {
          tabEdit.querySelector(".tab-label").textContent = inEdit ? "完成" : "编辑";
        }
      });
      obs.observe(editToggle, { attributes: true, attributeFilter: ["class"] });
    }
  }
  // ===== END TABLET UI =====
    });
